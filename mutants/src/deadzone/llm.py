"""Cliente de modelo com record/replay.

Duas garantias que o brief exige:

1. **Reprodução sem chave.** `DEADZONE_MODE=replay` (default) nunca toca a rede e
   não importa nenhuma dependência de terceiro — só a stdlib. Se faltar gravação,
   quebra alto: nunca silencia, nunca cai para chamada ao vivo, nunca inventa.
2. **Trajetória completa.** Cada chamada grava prompt, resposta, modelo, esforço,
   `stop_reason`, tokens, custo e timestamp em `recordings/`. Esse conjunto de
   arquivos *é* o artefato de trajetória do rubric.

O caminho ao vivo usa o SDK oficial `anthropic` (import tardio, só quando
`DEADZONE_MODE=live`). A chave é resolvida pelo próprio SDK a partir do
ambiente; nunca é lida, impressa, gravada nem incluída em nenhum artefato.

## Decisões de medição registradas aqui, não escondidas

- **`effort` fixo em `high` nos quatro estágios.** É um parâmetro que move
  qualidade e custo; variá-lo entre baseline e solução tornaria a comparação
  inválida. Fica gravado em cada chamada para poder ser auditado.
- **Sem `fallbacks` de recusa.** O SDK oferece troca automática de modelo quando
  um pedido é recusado. Num harness de medição isso seria um modelo diferente
  respondendo no meio da comparação, sem aviso — exatamente o tipo de variável
  não controlada que este projeto existe para expor. Em vez disso,
  `stop_reason == "refusal"` levanta erro e para a execução.
- **Reprodutibilidade honesta.** Opus 5 roda com pensamento adaptativo e não
  aceita `temperature`; duas execuções ao vivo do mesmo prompt podem divergir.
  Por isso o número reportado é o da **gravação**, e o caminho de reprodução do
  juiz é o replay. O caminho ao vivo regrava, não reconfere.
"""

from __future__ import annotations

import hashlib
import json
import os
import time
from dataclasses import dataclass, field
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
RECORDINGS = ROOT / "recordings"

DEFAULT_MODEL = "claude-opus-5"
DEFAULT_EFFORT = "high"
MAX_TOKENS = 48000
# Saída longa vai por streaming: a rodada de reparo devolve o teste inteiro de
# cada falha e estourou 16000 na primeira tentativa. Streaming evita o timeout
# de HTTP que um max_tokens alto provoca em requisição normal.

# USD por 1M tokens (entrada, saída). Fonte e data em REPRODUCTION.md § Cost model.
PRICING = {
    "claude-opus-5": (5.00, 25.00),
    "claude-fable-5": (10.00, 50.00),
    "claude-opus-4-8": (5.00, 25.00),
    "claude-sonnet-5": (2.00, 10.00),
    "claude-haiku-4-5": (1.00, 5.00),
}


from mutmut.mutation.trampoline import wrap_in_trampoline as _mutmut_mutated, MutantDict


class MissingRecording(RuntimeError):
    """Modo replay sem gravação. Falha alta — nunca conserte silenciosamente."""


class ModelRefused(RuntimeError):
    """O modelo recusou. Para a execução em vez de trocar de modelo por baixo."""


@dataclass
class Call:
    key: str
    provider: str
    model: str
    system: str
    prompt: str
    response: str = ""
    effort: str = DEFAULT_EFFORT
    thinking: str = "adaptive"
    stop_reason: str = ""
    input_tokens: int = 0
    output_tokens: int = 0
    cost_usd: float = 0.0
    cache_read_tokens: int = 0
    cache_write_tokens: int = 0
    wall_seconds: float = 0.0
    timestamp: str = ""
    stage: str = ""
    unit: str = ""
    error: str = ""

    def as_dict(self) -> dict:
        return self.__dict__.copy()
mutants_x__price__mutmut: MutantDict = {}  # type: ignore


@_mutmut_mutated(mutants_x__price__mutmut)
def _price(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_orig(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_1(model: str, tin: int, tout: int, cache_read: int = 1, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_2(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 1) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_3(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(None):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_4(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                None,
                6,
            )
    return 0.0


def x__price__mutmut_5(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                None,
            )
    return 0.0


def x__price__mutmut_6(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                6,
            )
    return 0.0


def x__price__mutmut_7(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                )
    return 0.0


def x__price__mutmut_8(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25 - cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_9(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout - cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_10(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin - tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_11(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 / pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_12(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin * 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_13(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1000001.0 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_14(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 / pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_15(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout * 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_16(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1000001.0 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_17(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin / 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_18(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 / pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_19(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write * 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_20(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1000001.0 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_21(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 2.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_22(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin / 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_23(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 / pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_24(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read * 1e6 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_25(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1000001.0 * pin * 0.10,
                6,
            )
    return 0.0


def x__price__mutmut_26(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 1.1,
                6,
            )
    return 0.0


def x__price__mutmut_27(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                7,
            )
    return 0.0


def x__price__mutmut_28(model: str, tin: int, tout: int, cache_read: int = 0, cache_write: int = 0) -> float:
    """Custo real, com os tokens de cache incluídos.

    A primeira versão somava só entrada e saída. Escrita de cache custa 1.25x a
    entrada e leitura custa 0.1x — omiti-las faz o relatório afirmar um custo
    menor que o cobrado, que é a direção errada para errar num item que o rubric
    pontua. Custou um contador que dizia US$ 4,25 quando a conta real passava de
    US$ 6,80.
    """
    for name, (pin, pout) in PRICING.items():
        if model.startswith(name):
            return round(
                tin / 1e6 * pin
                + tout / 1e6 * pout
                + cache_write / 1e6 * pin * 1.25
                + cache_read / 1e6 * pin * 0.10,
                6,
            )
    return 1.0

mutants_x__price__mutmut['_mutmut_orig'] = x__price__mutmut_orig # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_1'] = x__price__mutmut_1 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_2'] = x__price__mutmut_2 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_3'] = x__price__mutmut_3 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_4'] = x__price__mutmut_4 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_5'] = x__price__mutmut_5 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_6'] = x__price__mutmut_6 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_7'] = x__price__mutmut_7 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_8'] = x__price__mutmut_8 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_9'] = x__price__mutmut_9 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_10'] = x__price__mutmut_10 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_11'] = x__price__mutmut_11 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_12'] = x__price__mutmut_12 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_13'] = x__price__mutmut_13 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_14'] = x__price__mutmut_14 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_15'] = x__price__mutmut_15 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_16'] = x__price__mutmut_16 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_17'] = x__price__mutmut_17 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_18'] = x__price__mutmut_18 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_19'] = x__price__mutmut_19 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_20'] = x__price__mutmut_20 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_21'] = x__price__mutmut_21 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_22'] = x__price__mutmut_22 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_23'] = x__price__mutmut_23 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_24'] = x__price__mutmut_24 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_25'] = x__price__mutmut_25 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_26'] = x__price__mutmut_26 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_27'] = x__price__mutmut_27 # type: ignore # mutmut generated
mutants_x__price__mutmut['x__price__mutmut_28'] = x__price__mutmut_28 # type: ignore # mutmut generated
mutants_x_cache_key__mutmut: MutantDict = {}  # type: ignore


@_mutmut_mutated(mutants_x_cache_key__mutmut)
def cache_key(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps([provider, model, system, prompt, effort], sort_keys=True).encode()
    return hashlib.sha256(blob).hexdigest()[:20]


def x_cache_key__mutmut_orig(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps([provider, model, system, prompt, effort], sort_keys=True).encode()
    return hashlib.sha256(blob).hexdigest()[:20]


def x_cache_key__mutmut_1(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = None
    return hashlib.sha256(blob).hexdigest()[:20]


def x_cache_key__mutmut_2(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps(None, sort_keys=True).encode()
    return hashlib.sha256(blob).hexdigest()[:20]


def x_cache_key__mutmut_3(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps([provider, model, system, prompt, effort], sort_keys=None).encode()
    return hashlib.sha256(blob).hexdigest()[:20]


def x_cache_key__mutmut_4(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps(sort_keys=True).encode()
    return hashlib.sha256(blob).hexdigest()[:20]


def x_cache_key__mutmut_5(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps([provider, model, system, prompt, effort], ).encode()
    return hashlib.sha256(blob).hexdigest()[:20]


def x_cache_key__mutmut_6(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps([provider, model, system, prompt, effort], sort_keys=False).encode()
    return hashlib.sha256(blob).hexdigest()[:20]


def x_cache_key__mutmut_7(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps([provider, model, system, prompt, effort], sort_keys=True).encode()
    return hashlib.sha256(None).hexdigest()[:20]


def x_cache_key__mutmut_8(provider: str, model: str, system: str, prompt: str,
              effort: str = DEFAULT_EFFORT) -> str:
    """O esforço entra na chave: ele muda a resposta, não só o custo.

    Servir uma gravação feita em effort=high para um pedido em effort=low seria
    reportar um número que ninguém pode reproduzir com o comando publicado.
    """
    blob = json.dumps([provider, model, system, prompt, effort], sort_keys=True).encode()
    return hashlib.sha256(blob).hexdigest()[:21]

mutants_x_cache_key__mutmut['_mutmut_orig'] = x_cache_key__mutmut_orig # type: ignore # mutmut generated
mutants_x_cache_key__mutmut['x_cache_key__mutmut_1'] = x_cache_key__mutmut_1 # type: ignore # mutmut generated
mutants_x_cache_key__mutmut['x_cache_key__mutmut_2'] = x_cache_key__mutmut_2 # type: ignore # mutmut generated
mutants_x_cache_key__mutmut['x_cache_key__mutmut_3'] = x_cache_key__mutmut_3 # type: ignore # mutmut generated
mutants_x_cache_key__mutmut['x_cache_key__mutmut_4'] = x_cache_key__mutmut_4 # type: ignore # mutmut generated
mutants_x_cache_key__mutmut['x_cache_key__mutmut_5'] = x_cache_key__mutmut_5 # type: ignore # mutmut generated
mutants_x_cache_key__mutmut['x_cache_key__mutmut_6'] = x_cache_key__mutmut_6 # type: ignore # mutmut generated
mutants_x_cache_key__mutmut['x_cache_key__mutmut_7'] = x_cache_key__mutmut_7 # type: ignore # mutmut generated
mutants_x_cache_key__mutmut['x_cache_key__mutmut_8'] = x_cache_key__mutmut_8 # type: ignore # mutmut generated


@dataclass
class Client:
    provider: str = field(default_factory=lambda: os.environ.get("DEADZONE_PROVIDER", "anthropic"))
    model: str = field(default_factory=lambda: os.environ.get("DEADZONE_MODEL", DEFAULT_MODEL))
    mode: str = field(default_factory=lambda: os.environ.get("DEADZONE_MODE", "replay"))
    effort: str = field(default_factory=lambda: os.environ.get("DEADZONE_EFFORT", DEFAULT_EFFORT))
    recordings: Path = RECORDINGS
    calls: list[Call] = field(default_factory=list)
    cache_read: int = 0
    cache_write: int = 0
    _last_cache_read: int = 0
    _last_cache_write: int = 0

    def __post_init__(self) -> None:
        self.recordings.mkdir(parents=True, exist_ok=True)
        if self.mode not in ("replay", "live"):
            raise ValueError(f"DEADZONE_MODE inválido: {self.mode!r} (use replay|live)")

    # ---------------------------------------------------------------- público

    def complete(self, system: str, prompt: str, *, stage: str = "", unit: str = "",
                 cache_system: bool = False, effort: str | None = None) -> Call:
        """`cache_system` marca o bloco system para cache de prefixo.

        O system carrega o módulo e a suíte inteira — dezenas de milhares de
        tokens **idênticos** em toda chamada do mesmo módulo. Sem isso o custo
        por mutante inviabiliza o orçamento (medido no spike: US$ 0,10/chamada).
        A chave de cache do replay NÃO inclui esta flag: ela muda o custo, nunca
        a resposta, e uma gravação feita com cache é igual a uma feita sem.
        """
        effort = effort or self.effort
        key = cache_key(self.provider, self.model, system, prompt, effort)
        path = self.recordings / f"{key}.json"

        if path.exists():
            call = Call(**json.loads(path.read_text()))
            call.stage, call.unit = stage or call.stage, unit or call.unit
            self.calls.append(call)
            return call

        if self.mode == "replay":
            raise MissingRecording(
                f"sem gravação para {key} (stage={stage} unit={unit} model={self.model}).\n"
                f"Rode com DEADZONE_MODE=live e a chave no ambiente para gravar."
            )

        call = Call(key=key, provider=self.provider, model=self.model, system=system,
                    prompt=prompt, effort=effort, stage=stage, unit=unit)
        started = time.time()
        try:
            text, tin, tout, stop = self._live(system, prompt, cache_system=cache_system,
                                               effort=effort)
        except Exception as exc:  # noqa: BLE001 — o erro vira artefato, mas NUNCA cache
            call.error = f"{type(exc).__name__}: {exc}"
            # Chamada truncada em max_tokens GEROU saída e FOI COBRADA. Registrar
            # custo zero aqui é como o contador subestimou o gasto real: o custo
            # só era calculado no caminho de sucesso.
            call.input_tokens = getattr(exc, "_dz_in", 0)
            call.output_tokens = getattr(exc, "_dz_out", 0)
            call.cost_usd = _price(self.model, call.input_tokens, call.output_tokens)
            call.wall_seconds = round(time.time() - started, 3)
            call.timestamp = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
            self._record_failure(call)
            raise

        call.response, call.input_tokens, call.output_tokens = text, tin, tout
        call.stop_reason = stop
        call.wall_seconds = round(time.time() - started, 3)
        call.cache_read_tokens = self._last_cache_read
        call.cache_write_tokens = self._last_cache_write
        call.cost_usd = _price(self.model, call.input_tokens, call.output_tokens,
                               call.cache_read_tokens, call.cache_write_tokens)
        call.timestamp = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        path.write_text(json.dumps(call.as_dict(), indent=2, ensure_ascii=False) + "\n")
        self.calls.append(call)
        return call

    def _record_failure(self, call: Call) -> None:
        """Falha vai para recordings/failed/, nunca para o cache de replay.

        A distinção não é cosmética. Uma chamada que morreu com resposta vazia,
        gravada no cache, é indistinguível de um modelo que respondeu vazio: a
        próxima execução em replay serviria o vazio como se fosse resultado. É
        exatamente o modo de falha que este projeto existe para expor, e ele
        apareceu aqui primeiro. As falhas ficam registradas — o brief pede o
        histórico de erro — mas fora do caminho que alimenta a métrica.
        """
        failed = self.recordings / "failed"
        failed.mkdir(parents=True, exist_ok=True)
        stamp = time.strftime("%Y%m%dT%H%M%SZ", time.gmtime())
        (failed / f"{call.key}-{stamp}.json").write_text(
            json.dumps(call.as_dict(), indent=2, ensure_ascii=False) + "\n"
        )

    def totals(self) -> dict:
        return {
            "n_calls": len(self.calls),
            "input_tokens": sum(c.input_tokens for c in self.calls),
            "output_tokens": sum(c.output_tokens for c in self.calls),
            "cost_usd": round(sum(c.cost_usd for c in self.calls), 6),
            "wall_seconds": round(sum(c.wall_seconds for c in self.calls), 3),
            "model": self.model,
            "provider": self.provider,
            "mode": self.mode,
            "effort": self.effort,
            "cache_read_tokens": self.cache_read,
            "cache_write_tokens": self.cache_write,
        }

    # ----------------------------------------------------------------- rede

    def _live(self, system: str, prompt: str, *, cache_system: bool = False,
              effort: str | None = None) -> tuple[str, int, int, str]:
        if self.provider == "cursor":
            return self._live_cursor(system, prompt)
        if self.provider != "anthropic":
            raise NotImplementedError(
                f"provider {self.provider!r} não implementado. Este projeto mede um "
                f"modelo só, de propósito: trocar de provider no meio invalidaria a "
                f"comparação baseline↔solução. Para medir outro, rode a suíte inteira "
                f"de novo com DEADZONE_PROVIDER e grave em recordings/ separado."
            )

        import anthropic  # import tardio: o caminho de replay não depende disto

        client = anthropic.Anthropic()
        system_arg = (
            [{"type": "text", "text": system, "cache_control": {"type": "ephemeral"}}]
            if cache_system
            else system
        )
        with client.messages.stream(
            model=self.model,
            max_tokens=MAX_TOKENS,
            system=system_arg,
            thinking={"type": "adaptive"},
            output_config={"effort": effort or self.effort},
            messages=[{"role": "user", "content": prompt}],
        ) as stream:
            response = stream.get_final_message()

        if response.stop_reason == "refusal":
            detail = getattr(response, "stop_details", None)
            raise ModelRefused(
                f"modelo recusou (categoria={getattr(detail, 'category', None)}). "
                f"Nenhum fallback é acionado de propósito — trocar de modelo aqui "
                f"quebraria a comparação. Ver src/deadzone/llm.py."
            )
        if response.stop_reason == "max_tokens":
            err = RuntimeError(
                f"resposta truncada em max_tokens={MAX_TOKENS} (stage/unit na gravação). "
                f"Predição truncada não é predição ruim, é predição ausente — "
                f"aumente MAX_TOKENS e regrave, não pontue isto."
            )
            err._dz_in, err._dz_out = u.input_tokens, u.output_tokens
            raise err

        text = "".join(b.text for b in response.content if b.type == "text")
        u = response.usage
        # tokens lidos do cache custam ~0.1x; escritos custam ~1.25x. Somados à
        # entrada para que o custo reportado seja o real, não o otimista.
        self._last_cache_read = getattr(u, "cache_read_input_tokens", 0) or 0
        self._last_cache_write = getattr(u, "cache_creation_input_tokens", 0) or 0
        self.cache_read += self._last_cache_read
        self.cache_write += self._last_cache_write
        return text, u.input_tokens, u.output_tokens, response.stop_reason or ""

    def _live_cursor(self, system: str, prompt: str) -> tuple[str, int, int, str]:
        """Mesmo modelo, harness diferente — declarado, não escondido.

        Usado só quando o crédito de API acaba. O `cursor-agent` roda na
        assinatura do usuário, com system prompt e scaffolding próprios, então um
        número produzido aqui **não é comparável lado a lado** com um produzido
        pela API. Vai reportado à parte e rotulado (METRIC_TESTGEN.md § 13).

        `--mode ask` é obrigatório: sem ele o cursor-agent é um agente com
        ferramentas de escrita e shell, e um gerador de teste não pode ter
        permissão de editar o repositório que ele está medindo.
        """
        import shutil as _sh
        import subprocess

        exe = _sh.which("cursor-agent")
        if not exe:
            raise RuntimeError("cursor-agent não encontrado no PATH")

        import tempfile

        model = os.environ.get("DEADZONE_CURSOR_MODEL", "claude-opus-5-thinking-high")
        junto = f"{system}\n\n---\n\n{prompt}"
        # Roda num diretório vazio e descartável, nunca no repositório. `--mode ask`
        # já é somente-leitura; o diretório vazio é a segunda tranca, para que
        # `--trust` não signifique confiar com o repo que está sendo medido.
        with tempfile.TemporaryDirectory(prefix="deadbolt-cursor-") as vazio:
            proc = subprocess.run(
                [exe, "-p", "--output-format", "text", "--mode", "ask",
                 "--trust", "--model", model, junto],
                capture_output=True, text=True, timeout=1800, cwd=vazio,
            )
        if proc.returncode != 0:
            raise RuntimeError(f"cursor-agent falhou ({proc.returncode}): {proc.stderr[-400:]}")
        texto = proc.stdout.strip()
        if not texto:
            raise RuntimeError(f"cursor-agent devolveu vazio. stderr: {proc.stderr[-400:]}")
        # Sem contagem de tokens: assinatura não é medida por token, e inventar
        # um número aqui seria pior do que não ter nenhum.
        return texto, 0, 0, "end_turn"
