...
3 predições descartadas por âncora inválida:

- `slugify/slugify.py` [139, 140] — citação `except Exception:
            pass` não aparece no intervalo
- `slugify/slugify.py` [146, 147] — citação `text = HEX_PATTERN.sub(lambda m: chr(int(m.group(1), 16)), text)` não aparece no intervalo
- `slugify/slugify.py` [186, 188] — citação `# finalize user-specific replacements
    if replacements:` não aparece no intervalo

## Predições que sobreviveram

- `slugify/slugify.py:[30, 32]` · **default_argument** · conf 0.6 — slugify always passes word_boundary explicitly and both direct smart_truncate tests return before the word_boundary check, so the default value is never observed.
- `slugify/slugify.py:[49, 50]` · **boundary_condition** · conf 0.72 — Every test either has len(string) strictly below max_length or well above it; the only len == max_length case (max_length=19 with word_boundary) yields the identical result down the longer path, so `<` vs `<=` is indistinguishable.
- `slugify/slugify.py:[55, 56]` · **error_path** · conf 0.8 — This fallback is only reachable with word_boundary=True and a separator absent from the string, and the 'no separator' test returns earlier at the length check, so the guard never fires.
- `slugify/slugify.py:[60, 60]` · **error_path** · conf 0.6 — Inputs reaching the loop are always stripped and collapsed by slugify, so split() never yields an empty token and the false side of this guard is never taken.
- `slugify/slugify.py:[70, 71]` · **error_path** · conf 0.75 — No test uses a max_length smaller than the first word, so the empty-truncated fallback is never executed.
- `slugify/slugify.py:[114, 115]` · **error_path** · conf 0.88 — Every test passes a str literal, so the bytes-decoding fallback never executes and could be deleted or altered unnoticed.
- `slugify/slugify.py:[128, 129]` · **error_path** · conf 0.85 — unidecode/normalize always return str in the tested paths, so this second non-str guard is never taken.
- `slugify/slugify.py:[191, 191]` · **boundary_condition** · conf 0.6 — Tests use max_length=0 (default, skips) or clearly positive values; changing '>' to '>=' or to '!= 0' would not fail since smart_truncate returns the text unchanged for length 0.
- `slugify/special.py:[7, 8]` · **boundary_condition** · conf 0.4 — The function is only ever invoked on the three non-empty package default tables, so the empty-list (zero-iteration) case is never exercised and any change to how it is handled would go unnoticed.
- `slugify/special.py:[8, 8]` · **error_path** · conf 0.32 — Unpacking only ever sees well-formed 2-tuples from the internal tables, so the ValueError path for malformed replacement entries is never triggered by any test.
- `slugify/special.py:[10, 10]` · **unasserted_branch** · conf 0.72 — For every entry in the shipped CYRILLIC/GERMAN/GREEK tables where char equals its own uppercase (e.g. ('Ξ','X'), ('ϒ','Y')) the pair is already present in char_list, so the second conjunct never decides anything and could be deleted without changing the PRE_TRANSLATIONS list asserted in test_pre_translation.
