...
#
# Exige DEADBOLT_MODE=live e credencial no ambiente do PROCESSO (não do seu
# terminal interativo). Cada chamada é gravada em recordings/; rodar de novo
# depois disso é replay a custo zero.
#
#   DEADBOLT_MODE=live bash scripts/run_all_stages.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
for stage in baseline s4 s5 s6; do
  for set_name in dev holdout; do
    echo "=== $stage / $set_name"
    "$PY" -m deadbolt.predict --stage "$stage" --set "$set_name"
